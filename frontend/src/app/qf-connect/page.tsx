...
export default function QfConnectPage() {
  ...
  async function testBookmarks() {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/qf/user/bookmarks`, {
        credentials: "include",
        cache: "no-store",
      });
      const data = await res.json();
      ...
      <main className="mx-auto max-w-4xl px-5 py-8 sm:px-6 lg:px-8">
        <section className="rounded-[2rem] border border-[var(--border-soft)] bg-[var(--surface-raised)] p-6 shadow-[0_20px_60px_rgba(30,45,56,0.06)] sm:p-8">
          ...
          <p className="mt-3 text-sm leading-7 text-[var(--text-muted)]">
            Connecting Quran.Foundation allows Al-Huda to use Quran.Foundation
            user features such as bookmarks, goals, streaks, notes, and
            activity APIs.
          </p>
          ...
          <div className="mt-6 rounded-2xl border border-[var(--border-soft)] bg-[var(--surface-soft)] p-4">
            ...
            <p className="mt-2 text-sm text-[var(--text-strong)]">
              ...
              {status === "failed" &&
                "Connection failed. OAuth may be misconfigured (redirect URI/scopes)."}
            </p>
          </div>
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            ...
            <button
              ...
              disabled={loading}
              className="rounded-full border border-[var(--border-soft)] bg-[var(--surface-soft)] px-4 py-2 text-sm font-medium text-[var(--text-strong)] transition hover:bg-[var(--surface-raised)] disabled:opacity-60"
            >
          ...
          <div className="mt-6 rounded-2xl border border-[var(--border-soft)] bg-[var(--surface-soft)] p-4">
            <p className="text-sm font-semibold text-[var(--heading-accent)]">
              Bookmarks test result
            </p>
            <p className="mt-2 text-xs text-[var(--text-muted)]">
              This tests the Quran.Foundation User API through the Bookmarks
              endpoint.
            </p>
            <pre className="mt-3 max-h-72 overflow-auto rounded-xl bg-[var(--surface-raised)] p-3 text-xs text-[var(--text-strong)]">
